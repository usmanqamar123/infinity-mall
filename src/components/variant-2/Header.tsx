...
const Header = () => {
  ...
  return (
    <>
      {/* Desktop Nav */}
      <nav
        className={`w-full fixed top-0 z-40 px-10  hidden xl:flex items-center justify-between transition-all duration-300 ${
          scrolled
            ? "bg-gray-950 shadow-md border-b border-white/25"
            : "bg-transparent"
        }`}
      >
              ...
              (Icon, idx) => (
                <Icon
                  ...
                  className={`transition-colors duration-300 cursor-pointer ${
                    scrolled ? "text-white" : "text-white hover:text-[#008B8B]"
                  } `}
                />
              )
            )}
