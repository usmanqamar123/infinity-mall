...
const ImageGrid: React.FC = () => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 grid-rows-2 gap-0">
      {gridItems.map((item) => (
        <div
          key={item.id}
          className={`overflow-hidden relative h-64 md:h-96 group `}
        >
          <Image
            src={item.image}
            alt={item.text}
            ...
            priority={item.id === 1}
          />
          <div className="absolute inset-0 bg-gradient-to-b from-white/0 to-[#1A1A1A]/80 flex flex-col items-center justify-center">
